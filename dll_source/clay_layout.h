...
#define EXPORT_DEFAULT DLLEXPORT double
EXPORT_DEFAULT gml_clay_element_config(){
    Clay__OpenElement();
    Clay__ConfigureOpenElementDynamic(current_elemet);
    return 1;
}
EXPORT_DEFAULT gml_clay_ui_set_scrolling(double horizontal, double vertical){
    current_elemet.scroll.horizontal = (bool)horizontal;
    current_elemet.scroll.vertical= (bool)vertical;
    return 1;
  }
EXPORT_DEFAULT gml_clay_ui_element_set_border( double border_w){
...
EXPORT_DEFAULT gml_clay_ui_element_set_color( double color, double alpha){
...
EXPORT_DEFAULT gml_clay_ui_element_set_image_id( double image_id){
...
EXPORT_DEFAULT gml_clay_set_text_spacing(double f_space, double f_line_h,double f_wrapmode){
...
EXPORT_DEFAULT gml_clay_set_text_font(double f_id, double f_size){
    current_text_config.fontId = (u16)f_id;
    current_text_config.fontSize = (u16)f_size;
    return 1;
}
EXPORT_DEFAULT gml_clay_set_text_color(double color, double alpha){
    UniColor _col= {.uni = (u32)color};
    current_text_config.textColor = UNI_COLOR_TO_CLAY_COLOR(_col);
    current_text_config.textColor.a = 255 * alpha;
    return 1;
}
EXPORT_DEFAULT gml_clay_element_set_text(char *gml_string){
...
EXPORT_DEFAULT gml_clay_begin_layout(){
    open_elements = 0;
...
EXPORT_DEFAULT gml_clay_end_layout(double update_commands){
    //m_requests.counter = 0;
...
EXPORT_DEFAULT gml_clay_open_ui_element(char *name){
    ZeroMemory_Util(&current_elemet, sizeof(Clay_ElementDeclaration));
    open_elements++;
...
EXPORT_DEFAULT gml_clay_set_ui_element(){
    if( open_elements)Clay__ConfigureOpenElementDynamic(current_elemet);
    if(open_elements) Clay__CloseElement();
    open_elements--;
    return 1;
}
EXPORT_DEFAULT gml_clay_close_ui_element(){
    Clay__CloseElement();
    return 1;
}
EXPORT_DEFAULT gml_clay_ui_element_background_color(double color, double alpha){
    UniColor _col= {.uni = (u32)color};
...
EXPORT_DEFAULT gml_clay_ui_element_layout_sizing(double w_type, double h_type, double w_var, double h_var){
    switch((u32)w_type){
...
EXPORT_DEFAULT gml_clay_ui_element_set_border_radius(double radius){
    current_elemet.cornerRadius = CLAY_CORNER_RADIUS(radius);
    return 1;
}
EXPORT_DEFAULT gml_clay_ui_element_layout_padding(double bottom, double left, double right, double top){
...
EXPORT_DEFAULT gml_clay_ui_element_layout_padding_all(double padding){
    current_elemet.layout.padding = CLAY_PADDING_ALL(padding);
    return 1;
}
EXPORT_DEFAULT gml_clay_ui_element_layout_child(double child_gap,double allign_x, double allign_y){
...
EXPORT_DEFAULT gml_clay_ui_element_layout_direction(double dir){
